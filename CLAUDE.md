...
Данные о топливе — с 24.07.2026 единственный источник истины во всей системе (аналитика,
отчёты, себестоимость заявки, карточка машины, Excel-экспорт) — `VehicleTrip`/Wialon:
`calculatedFuelConsumedL`/`calculatedKm` (расход), `fuelCostAmd` (стоимость). **`FleetExpense`
(даже строки `expenseType='fuel'`) — отдельный расходный поток, никогда не подмешивается в
«Топливо»** — это конвенция эталонного `app/api/reports/own-fleet/route.ts`, легко нарушить
по ошибке при добавлении нового потребителя. `FuelRecord` — только ручной журнал заправок на
`/fuel`, больше нигде не читается. Подробности: `docs/fuel-data-migration.md`.
...
## Язык общения
Всегда отвечай пользователю на русском языке в чате и объяснениях.
Код, комментарии в коде, названия коммитов, названия переменных и функций —
оставляй на английском языке, как принято в разработке.
Эта инструкция должна применяться во всех последующих сессиях.
